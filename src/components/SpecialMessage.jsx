...
const SpecialMessage = () => {
  ...
  const { specialMessage } = birthdayContent;
  const [text, setText] = useState(specialMessage.text);

  useEffect(() => {
    if (inView) {
      setText(specialMessage.text);
    }
  }, [inView, specialMessage.text]);
        ...
        >
          <p
            style={{
              ...
              color: "#fff",
              textShadow:
                "0 0 18px rgba(255, 75, 130, 0.45), 0 2px 8px rgba(0,0,0,0.5)",
              whiteSpace: "pre-wrap",
              textAlign: "left",
              letterSpacing: "0.3px",
              paddingLeft: "12px",
